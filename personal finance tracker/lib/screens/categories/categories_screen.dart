  ...
  Widget build(BuildContext context) {
    return Consumer2<CategoryProvider, TransactionProvider>(
      builder: (context, categoryProvider, transactionProvider, child) {
        ...
        final totalExpense = transactionProvider.expenseByCategory.values.fold(0.0, (sum, amount) => sum + amount);
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final expenseAmount = transactionProvider.expenseByCategory[category.name] ?? 0.0;
            final incomeAmount = transactionProvider.transactions
                .where((t) => t.type == 'income' && t.category == category.name)
                .fold(0.0, (sum, t) => sum + t.amount);
            
            final expensePercentage = totalExpense > 0 ? (expenseAmount / totalExpense * 100) : 0.0;
            final incomePercentage = totalIncome > 0 ? (incomeAmount / totalIncome * 100) : 0.0;
  ...
  Widget _buildPercentageRow() {
    ...
    if (category.type == 'expense' || category.type == 'both') {
      if (expensePercentage > 0) {
        percentageWidgets.add(
          Text(
            'مصروف: ${expensePercentage.toStringAsFixed(1)}%',
    ...
    if (category.type == 'income' || category.type == 'both') {
      if (incomePercentage > 0) {
        if (percentageWidgets.isNotEmpty) {
          percentageWidgets.add(const SizedBox(width: 8));
        }
        percentageWidgets.add(
          Text(
            'دخل: ${incomePercentage.toStringAsFixed(1)}%',
            style: const TextStyle(
              color: Colors.green,
              fontSize: 11,
              fontWeight: FontWeight.w500,
    ...
    return Row(children: percentageWidgets);
  }
...
class _CategoryFormSheetState extends State<_CategoryFormSheet> {
  ...
  /// Creates a Category object from form fields
  ...
  void _showError(dynamic error) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('فشل في حفظ التصنيف: $error')));
    }
  }
